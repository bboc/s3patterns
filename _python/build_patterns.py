...
def create_source_files_for_slides(args):
    ...
    def make_file(root, filename_root, title_root):
        """Create file if it does not exist ('x' is exclusive creation)."""
        with file(os.path.join(root, '%s.md' % make_pathname(filename_root)), 'x') as fp:
    ...
    for group in s3_patterns.keys():
        ...
        for pattern in s3_patterns[group]:
            make_file(group_root, patter, pattern)
